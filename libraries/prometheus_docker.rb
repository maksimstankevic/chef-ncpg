class Chef
  class Resource
    # resource for prometheus installing/configuring via docker
    class PrometheusDocker < BaseDocker
      ...
      attribute(
        :user,
        kind_of: String,
        default: lazy { node['chef-ncpg']['prometheus']['user'] }
      )

      attribute(
        :group,
        kind_of: String,
        default: lazy { node['chef-ncpg']['prometheus']['group'] }
      )

      attribute(
        :version,
        kind_of: String,
        default: lazy { node['chef-ncpg']['prometheus']['version'] }
      )

      attribute(
        :version_lock,
        kind_of: [TrueClass, FalseClass],
        default: lazy { node['chef-ncpg']['prometheus']['version_lock'] }
      )

      attribute(
        :port,
        kind_of: String,
        default: lazy { node['chef-ncpg']['prometheus']['port'] }
      )
      ...
      attribute(
        ...
        kind_of: String,
        default:
        lazy { node['chef-ncpg']['docker']['prometheus']['docker_host_port'] }
      )
      ...
      attribute(
        ...
        kind_of: String,
        default:
        lazy { node['chef-ncpg']['docker']['prometheus']['container_ip'] }
      )

      attribute(
        :args,
        kind_of: Array,
        default: lazy { node['chef-ncpg']['prometheus']['args'] }
      )
    end
  end
  ...
  class Provider
    ...
    class PrometheusDocker < BaseDocker
      ...
      def prometheus_conf_file
        bridge_ip = new_resource.docker_bridge_ip.gsub(%r{\/.*}, '')
        node_exporter_port_arr = node['chef-ncpg']['node_exporter']['args'].dup
        node_exporter_port = \
          node_exporter_port_arr.grep(/^--web.listen-address=/)[0]
                                .match(/[0-9]+/)[0]
      ...
      def prometheus_container
        ...
        docker_image 'prometheus_image' do
